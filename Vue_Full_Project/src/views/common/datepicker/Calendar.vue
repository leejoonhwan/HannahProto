<template>
  <table class="table-condensed">
    ...
    <tr v-for="dateRow in calendar" :key="calendar.indexOf(dateRow)">
      <slot name="date-slot" v-for="date in dateRow" >
        <td :key="dateRow.indexOf(date)" :class="dayClass(date)" @click="$emit('dateClick', date)" @mouseover="$emit('hoverDate', date)">{{date | dateNum}}
        </td>
      </slot>
    </tr>
    </tbody>
